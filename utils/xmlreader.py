...
def combineparse2sent(sent, parse):
    ...
    while parse:
        ...
        word = str(tokenlist[tidx]).replace(' ','')
    ...
    with open(fconll, 'w') as fout:
        for sent in sentlist:
            for token in sent.tokenlist:
                # print('word: ',str(sent.idx))
                # print('lemma: ',token.lemma)
                #注意这里的token.word和token.lemma被我强制转换成str了
                line = str(sent.idx) + '\t' + str(token.idx) + '\t' + str(token.word,encoding='utf-8') + '\t' + str(token.lemma,encoding='utf-8') + '\t' + str(token.pos,encoding='utf-8') + '\t' + str(token.deptype) + '\t' + str(token.headidx) + '\t' + str(token.nertype,encoding='utf-8') + '\t' + str(token.partialparse) + '\n'
...
if __name__ == '__main__':
    sentlist, constlist = reader('test1.xml')
    sentlist = combine(sentlist, constlist)
    writer(sentlist, 'test1.conll')