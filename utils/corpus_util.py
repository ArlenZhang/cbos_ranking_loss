...
def get_text_by_edu(edus_pkl):
    edus_list = get_edus(edus_pkl)
    # 拼接文本数据
    result_text = ""
...
# 获取edus列表
def get_edus(edus_pkl):
    if os.path.exists(edus_pkl):
        # 直接加载数据
        edus_list = load_data(edus_pkl)
    else:
        edus_list = None
        input("请先执行prepare_data.py生成每个语料的edu信息")
    return edus_list
